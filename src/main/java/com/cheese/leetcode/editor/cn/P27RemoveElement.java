...
public class P27RemoveElement {
    public static void main(String[] args) {
        ...
        // TO TEST
        int count = solution.removeElement(new int[]{3, 2, 2, 3}, 2);
    }
    ...
    class Solution {
        ...
        public int removeElement(int[] nums, int val) {
            return removeElement_(nums, val);
        }

        /**
         * 指针！
         * 指针1来定位非target的元素的位置：提供
         * 指针2来遍历数组
         *
         * @param nums
         * @param val
        ...
        public int removeElement_(int[] nums, int val) {
            int x = 0, y = 0;
            while (y < nums.length) {
                //如果右侧指针元素与目标值不同，则将不同的值赋值给左侧指针，左右侧指针 ++  相同时仅右侧指针往右遍历 直到遍历所有的元素
                if (nums[y] != val) {
                    nums[x] = nums[y];
                    x++;
                }
